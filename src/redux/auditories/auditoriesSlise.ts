...
import {
  ...
  AuditoriesInitialState,
  AuditoriesTypes,
} from "./auditoriesTypes";
...
const auditoriesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      deleteAuditoryCategory.fulfilled,
      (state, action: PayloadAction<number>) => {
        if (!state.auditoriCategories) return;

        const newCategories = state.auditoriCategories.filter(
    ...
    builder.addCase(
      createAuditory.fulfilled,
      (state, action: PayloadAction<AuditoriesTypes>) => {
        if (!state.auditoriCategories) return;
    ...
    builder.addCase(
      deleteAuditory.fulfilled,
      (state, action: PayloadAction<number>) => {
        if (!state.auditoriCategories) return;

        const updatedCategories = state.auditoriCategories.map((el) => {
          const newAuditories = el.auditories.filter(
            (auditory) => auditory.id !== action.payload
          );
